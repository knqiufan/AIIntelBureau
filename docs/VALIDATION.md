...
- 后端：`python -m pytest`、`python -m app.smoke_memory --in-memory`、`python -m app.rehearse --in-memory --runs 20`、`python -m app.smoke_deepagents`。
...
- 可选活动口令：独立浏览器用例验证输入口令后建立带 TTL、可撤销的 HttpOnly Cookie 会话；写操作同时校验 CSRF token，浏览器不会保留原始口令。
...
- 自由耳语默认关闭；显式启用后会拒绝手机号、身份证号、邮箱、银行卡、地址信号、提示词注入及 `.env` 配置的敏感词；拒绝响应不会回显原文，也不会写入事件账本。
...
本工作区未安装 Docker，因此本机不能执行镜像构建或 Compose 启动；GitHub Actions 的 `CI quality gates` 会验证 Compose 模型与镜像构建。云端 OceanBase/seekdb、StepFun LLM/embedding 也尚未填入凭据，因此尚未进行真实服务调用。
